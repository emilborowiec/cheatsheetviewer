...
using CheatSheetViewerApp.CheatSheetViewComponent;
    ...
    public class MainWindowViewModel : INotifyPropertyChanged
    {
            ...
            if (Errors.Count > 0)
            {
                Dispatcher.CurrentDispatcher.BeginInvoke(
                    (Action) (async () =>
                    {
                        ...
                        OpenDialog(
                            "Some CheatSheetMarkup files failed to load!",
            ...
            } else if (_cheatSheets.Count == 0)
            {
                Dispatcher.CurrentDispatcher.BeginInvoke(
                    (Action) (async () =>
                    {
                        await Task.Delay(500);
                        OpenDialog(
                            "No CheatSheetMarkup files found!",
                            "CheatSheetMarkup files are loaded from user's 'Documents\\My CheatSheets' folder.\nCheck online help (F1) for samples and file format documentation.");
                    }));
            }
        }
        ...
        private void ShowOnlineHelp()
        {
            ...
            try
            {
                ...
                myProcess.StartInfo.FileName = "https://github.com/emilborowiec/cheatsheetviewer/wiki";
                myProcess.Start();
            }
            catch (Exception e)
            {
